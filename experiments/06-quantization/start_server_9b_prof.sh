...
export VLLM_TORCH_PROFILER_DIR=/var/tmp/vllm-study/traces_9b
DIR=/home/mpathak/code/research/serving/qwen35
mkdir -p /var/tmp/vllm-study/traces_9b

tmux kill-session -t vllm9b 2>/dev/null
sleep 2
...
tmux new-session -d -s vllm9b \
  "export HF_HOME=/var/tmp/vllm-study/hf-cache VLLM_WORKER_MULTIPROC_METHOD=spawn FLASHINFER_DISABLE_VERSION_CHECK=1 PATH=/var/tmp/vllm-study/venv-qwen35/bin:\$PATH; \
   /var/tmp/vllm-study/venv-qwen35/bin/vllm serve QuantTrio/Qwen3.5-9B-AWQ \
     --dtype float16 --max-model-len 8192 --max-num-seqs 256 \
     --gpu-memory-utilization 0.9 --trust-remote-code --port 8000 \
     --profiler-config.profiler=torch \
     --profiler-config.torch_profiler_dir=/var/tmp/vllm-study/traces_9b \
     > $DIR/steady_9b_server.log 2>&1"
echo "launched tmux session vllm9b"
sleep 2
tmux ls
