...
image: /assets/diagrams/d7-admit-vs-evict.jpg
---

*Part 5 of "The Inference Wall". A step back onto the series' rig, with a dense companion
alongside: Qwen3.5-4B and Qwen2.5-7B on a single NVIDIA A10G (23 GB), served under vLLM.*

*Manas Pathak · September 25, 2026*
...
**Evict-capable** engines admit optimistically and pull work back under pressure. vLLM admits
requests without reserving room for their eventual output, and when the pool runs dry mid-decode
it preempts a running request. SGLang does the same in two stages: it first evicts unreferenced
...
is saved, so the recompute is the recovery.
...
tokens and collectively they no longer fit. When an active decode step needs one more block and
...
The counts also move exactly the way the mechanism says they should. Squeeze the pool and the
recompute path works harder: the dense 7B model under the same variable workload preempts 101
times with its natural cache and 173 times when I cap the cache to seat barely two requests.
Adding output-length variance does the same. The same 7B model with the same natural cache
preempts 60 times under fixed-length outputs and 101 under variable-length ones, because variance
...
On the tight 7B cache, the count climbs monotonically the whole run (2, 6, 12, 25, 40, on up to
181) while the running set stays pinned at three to seven requests. There is no settling: the
cache is small enough that the server is permanently over-committed relative to demand, so it
evicts and recomputes continuously as a steady-state cost of operating past its comfortable
...
capped tight on the hybrid model, to 5.7 minutes on the tightest dense arm. The 99th-percentile
end-to-end latency on that tightest arm is **12.9 minutes** for a request that would complete in
seconds on an unloaded server. That is recompute work plus queueing, not a crash, but for an
interactive workload it is indistinguishable from an outage.
...
each flood, never the server-log clause, which cannot print in this version. The five-arm
sweep (`run_counter_remeasure.sh`), the positive control (`run_preempt_positive_control.sh`),
and the recompute cross-check with its three-second metric timelines
(`run_recompute_crosscheck.sh`) are in
[`experiments/05-cache-starvation/`](https://github.com/mapathak-commits/inference-wall/tree/main/experiments/05-cache-starvation);
the raw `remeasure.log`, `pc_driver.out`, `crosscheck.log`, and per-regime `cc_timeline_*.log`
files they produced are in
[`benchmarks/05-cache-starvation/`](https://github.com/mapathak-commits/inference-wall/tree/main/benchmarks/05-cache-starvation).
